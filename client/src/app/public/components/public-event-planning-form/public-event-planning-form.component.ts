...
export class PublicEventPlanningFormComponent {
  ...
  phone():string{return this.settings()?.commercialContact?.phone||this.settings()?.contact?.phones?.[0]||'064/803 0973';} email():string{return this.settings()?.commercialContact?.email||this.settings()?.contact?.generalEmail||'sales@madlenianum.rs';} contactName():string{return this.settings()?.commercialContact?.contactName||'Jana Aksentijević';} responseText():string{return this.settings()?.commercialContact?.responseTimeText||'naš tim će Vam odgovoriti u roku od 24 sata.';}
