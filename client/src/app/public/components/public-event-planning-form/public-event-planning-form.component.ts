...
export class PublicEventPlanningFormComponent {
  readonly spaces=input<RentalSpace[]>([]);readonly settings=input<PublicSiteSettings|null>(null);private readonly fb=inject(FormBuilder);private readonly api=inject(PublicApiService);readonly submitting=signal(false);readonly error=signal('');readonly reference=signal('');readonly emailWarning=signal(false);private idempotencyKey=this.newKey();
  ...
  submit():void{if(this.form.invalid||this.submitting())return this.form.markAllAsTouched();const raw=this.form.getRawValue();this.submitting.set(true);this.error.set('');this.api.createEventPlanningInquiry({...raw,preferredRentalSpace:raw.preferredRentalSpace||undefined,desiredDate:raw.desiredDate||undefined,approximateGuestCount:raw.approximateGuestCount||undefined,idempotencyKey:this.idempotencyKey}).pipe(finalize(()=>this.submitting.set(false))).subscribe({next:r=>{this.reference.set(r.item.referenceNumber);this.emailWarning.set(['failed','not_configured'].includes(r.emailStatus||''));this.form.reset();this.idempotencyKey=this.newKey();},error:e=>this.error.set(e?.error?.message||'Upit nije moguće poslati. Proverite podatke i pokušajte ponovo.')});}
  phone():string{return this.settings()?.commercialContact?.phone||this.settings()?.contact?.phones?.[0]||'';} email():string{return this.settings()?.commercialContact?.email||this.settings()?.contact?.generalEmail||'';} contactName():string{return this.settings()?.commercialContact?.contactName||'Sektor komercijalnih poslova';} responseText():string{return this.settings()?.commercialContact?.responseTimeText||'Naš tim će vam odgovoriti u najkraćem roku.';}
