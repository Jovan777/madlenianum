...
export class ReviewLinksEditorComponent{
  ...
  add(value:Record<string,unknown>={}):void{this.array.push(this.fb.group({title:[String(value['title']||'')],publication:[String(value['publication']||'')],url:[String(value['url']||''),[Validators.required,Validators.pattern(/^https?:\/\//i)]],publishedAt:[this.dateValue(value['publishedAt'])],note:[String(value['note']||'')],translations:this.fb.group({en:this.fb.group({title:[''],publication:[''],note:['']})}),displayOrder:[this.array.length]}));this.array.markAsDirty();}
