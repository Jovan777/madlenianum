...
export class CastEditorComponent {
  ...
  add(value: Record<string, unknown> = {}): void { this.array.push(this.fb.group({ artist:[String(value['artist']||'')], name:[String(value['name']||'')], role:[String(value['role']||value['character']||'')], note:[String(value['note']||'')], displayOrder:[this.array.length] })); this.array.markAsDirty(); }
