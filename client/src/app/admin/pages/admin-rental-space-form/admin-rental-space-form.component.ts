...
import { GalleryItemInput, CmsListItem } from '../../../core/models/cms.models';
...
@Component({
  selector: 'app-admin-rental-space-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './admin-rental-space-form.component.html',
  styleUrl: './admin-rental-space-form.component.scss',
})
export class AdminRentalSpaceFormComponent implements OnInit {
  ...
  readonly heroImage = signal<MediaSelectionValue[]>([]);
  readonly floorPlan = signal<MediaSelectionValue[]>([]);
  readonly venues = signal<CmsListItem[]>([]);
  ...
  readonly form = this.fb.group({
    ...
    seatedCapacity: [0, [Validators.min(0)]],
    standingCapacity: [0, [Validators.min(0)]],
    areaSqm: [0, [Validators.min(0)]],
    amenities: this.fb.control<string[]>([]),
    technicalEquipment: this.fb.control<string[]>([]),
    suitableEventTypes: this.fb.control<string[]>([]),
    accessibilityInfo: [''],
    dressingRooms: [''],
    cateringInfo: [''],
    barInfo: [''],
  ...
  ngOnInit(): void {
    ...
    this.cms
      .list<CmsListItem>('venues', { limit: 100 })
      .subscribe({ next: (r) => this.venues.set(r.items) });
  ...
  updateMedia(field: 'heroImage' | 'floorPlanPdf', selection: MediaSelectionResult): void {
    ...
    if (field === 'heroImage') this.heroImage.set(value);
    else this.floorPlan.set(value);
  }
  preview(): void {
    if (this.id()) {
      window.open(
        `/admin/rental-spaces/${this.id()}/preview?lang=${this.activeLanguage()}`,
        '_blank',
        'noopener',
      );
    }
  ...
  save(status?: 'draft' | 'published'): void {
    ...
    if (this.form.invalid || this.saving()) {
      ...
      this.error.set('Naziv prostora je obavezan.');
      return;
    }
    ...
    const payload = {
      ...
      floorPlanPdf: raw.floorPlanPdf || null,
      linkedVenue: raw.linkedVenue || null,
      publishedAt: raw.publishedAt ? new Date(raw.publishedAt).toISOString() : undefined,
    };
    const request = this.id()
      ? this.api.updateRentalSpace(this.id(), payload)
      : this.api.createRentalSpace(payload);
    request.pipe(finalize(() => this.saving.set(false))).subscribe({
      next: (r) => {
        this.notifications.success('Prostor je sačuvan.');
        this.patch(r.item);
        if (!this.id()) {
          this.id.set(r.item.id);
          this.router.navigate(['/admin/rental-spaces', r.item.id, 'edit'], { replaceUrl: true });
        }
      },
      error: (e) => {
  ...
  private load(): void {
    ...
    this.api
      .getRentalSpace(this.id())
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
        next: (r) => this.patch(r.item),
        error: (e) => this.error.set(e?.error?.message || 'Prostor nije učitan.'),
      });
  }
  private patch(item: RentalSpace): void {
    const seo = item.seo || {};
    const english = (item.translations?.en || {}) as Record<string, unknown>;
    const hero = this.mediaId(item.heroImage),
      pdf = this.mediaId(item.floorPlanPdf);
    this.form.patchValue({
      ...item,
      heroImage: hero,
      floorPlanPdf: pdf,
      linkedVenue: item.linkedVenue?.id || '',
      ...
      translations: {
        en: {
          ...
          amenities: this.strings(english['amenities']),
          technicalEquipment: this.strings(english['technicalEquipment']),
          suitableEventTypes: this.strings(english['suitableEventTypes']),
          accessibilityInfo: String(english['accessibilityInfo'] || ''),
          dressingRooms: String(english['dressingRooms'] || ''),
          cateringInfo: String(english['cateringInfo'] || ''),
          barInfo: String(english['barInfo'] || ''),
          internetInfo: String(english['internetInfo'] || ''),
          avInfo: String(english['avInfo'] || ''),
          seoTitle: String(english['seoTitle'] || ''),
          seoDescription: String(english['seoDescription'] || ''),
        },
      },
    });
    this.heroImage.set(hero ? [hero] : []);
    this.floorPlan.set(pdf ? [pdf] : []);
